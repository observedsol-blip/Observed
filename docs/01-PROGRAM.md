...
- Prüft `round_id == next_round_id ≤ max_round_id`, dann `RoundTerms::validate`: `version == 3`, `source_kind == 1`, `kind` ∈ {ABOVE, MOVE} (ABOVE: `offset_bps > −10 000`; MOVE: `0 < offset_bps < 10 000`), `max_conf_bps > 0`, `1 ≤ W, A ≤ 3 600`, **`reference_time − A > commit_close`** (kein zulässiger Referenzwert kann während des Siegelns veröffentlicht worden sein; Fehler `ReferenceBeforeSealCloses`), `commit_open < commit_close` und `reference_time + W < outcome_time`. Setzt `reveal_close = outcome_time + 12 h`, `resolve_deadline = outcome_time + 24 h`.
- **`terms_hash` kanonisch (v3)**, Byte für Byte: `sha256("observed/terms/v3" ‖ season_u16_le ‖ round_id_u32_le ‖ version_u8 ‖ kind_u8 ‖ source_kind_u8 ‖ feed_id[32] ‖ price_account[32] ‖ offset_bps_i32_le ‖ max_conf_bps_u16_le ‖ window_secs_u16_le ‖ max_age_secs_u16_le ‖ commit_open_i64_le ‖ commit_close_i64_le ‖ reference_time_i64_le ‖ outcome_time_i64_le)` — ohne Schwelle und ohne Text. Alles, was die Auswertung bestimmt, steht darin.
