...
- Prüft `round_id == next_round_id ≤ max_round_id`, dann `RoundTerms::validate`: `version == 3`, `source_kind == 1`, `kind` ∈ {ABOVE, MOVE} (ABOVE: `offset_bps > −10 000`; MOVE: `0 < offset_bps < 10 000`), `max_conf_bps > 0`, `0 < band_bps < |offset_bps|` (Fehler `BadBand`), `1 ≤ W, A ≤ 3 600`, **`reference_time − A > commit_close`** (kein zulässiger Referenzwert kann während des Siegelns veröffentlicht worden sein; Fehler `ReferenceBeforeSealCloses`), `commit_open < commit_close` und `reference_time + W < outcome_time`. Setzt `reveal_close = outcome_time + 12 h`, `resolve_deadline = outcome_time + 24 h`.
