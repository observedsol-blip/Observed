...
- Nur wenn `next_round_id > max_round_id` (vorherige Saison vollständig) oder erste Saison. Setzt `season`, `calendar_root`, `first_round_id = next_round_id`, `max_round_id = first_round_id + leaf_count − 1` (`leaf_count ≤ 64`). Dieselben Blobs (bis zu 64, Tiefe 6) liegen in `CALENDAR.md`.
