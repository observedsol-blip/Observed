...
**Round** (498 B inkl. Diskriminator, im Test `account_sizes_are_pinned` festgenagelt): `round_id u32`, `terms_hash [u8;32]`, `version u8`, `kind u8` {0 ABOVE, 1 MOVE}, `source_kind u8` {1 = Preiskonto}, `feed_id [u8;32]`, `price_account Pubkey` (das eine Konto, aus dem gelesen werden darf), `offset_bps i32`, `max_conf_bps u16`, `band_bps u16` (Messbandbreite, Saison 1: 25), `window_secs u16` (W), `max_age_secs u16` (A), `commit_open i64`, `commit_close i64`, `reference_time i64` (R), `outcome_time i64` (T), `reveal_close i64`, `resolve_deadline i64` (T+86 400), `status u8` {Open, Closed, Referenced, Resolved, Cancelled}, `outcome u8` {Unset, Yes, No}, `reference Reading`, `evidence Reading`, `threshold_mantissa i64` und `threshold_low_mantissa i64` (nur Anzeige; entschieden wird exakt, siehe `resolve`), `outcome_margin_bps i32` (wie weit der Ausgang die Schwelle verfehlt oder überschritten hat, vorzeichenbehaftet; `|outcome_margin_bps| ≤ band_bps` heißt: innerhalb der Messbandbreite entschieden), `commit_count u32`, `reveal_count u32`, `histogram [u32;21]`, `reserved [u8;32]`, `bump`. Kein Fragetext on-chain — der Client erzeugt ihn aus Regel und Zahlen.
...
**Player**: `sgt_mint`, `commits u32`, `reveals u32`, `missing_scored u32`, `score_sum u64`, `scored_rounds u32`, `bump`. Kein `missing`-Feld: Missing = commits − reveals − offene Einträge, vom Client abgeleitet (offene = Einträge in Runden, deren `reveal_close` noch nicht erreicht ist; das sind höchstens drei, seit das Aufdeckfenster 72 h dauert).
...
- Prüft `round_id == next_round_id ≤ max_round_id`, dann `RoundTerms::validate`: `version == 3`, `source_kind == 1`, `kind` ∈ {ABOVE, MOVE} (ABOVE: `offset_bps > −10 000`; MOVE: `0 < offset_bps < 10 000`), `max_conf_bps > 0`, `band_bps > 0`, und für Bewegungsfragen zusätzlich `band_bps < |offset_bps|`; bei der **Richtungsfrage** (ABOVE mit `offset_bps == 0`) gilt stattdessen `band_bps ≤ MAX_DIRECTION_BAND_BPS` (100) — dort ist das Band kein Abstand zu einer Schwelle, sondern nur die Marke für „im Messrauschen entschieden“ (Fehler `BadBand`), `1 ≤ W, A ≤ 3 600`, **`reference_time − A > commit_close`** (kein zulässiger Referenzwert kann während des Siegelns veröffentlicht worden sein; Fehler `ReferenceBeforeSealCloses`), `commit_open < commit_close` und `reference_time + W < outcome_time`. Setzt `reveal_close = outcome_time + 72 h` (`REVEAL_WINDOW_SECS`, seit 21.09.2026: ein ausgelassener Tag darf keinen vollen Fehlschlag kosten), `resolve_deadline = outcome_time + 24 h`. Die Länge ist eine Programmkonstante und steht nicht in den Bedingungen — `reveal_close` selbst steht aber in jeder Runde und ist damit pro Runde nachprüfbar.
- **`terms_hash` kanonisch (v3)**, Byte für Byte: `sha256("observed/terms/v3" ‖ season_u16_le ‖ round_id_u32_le ‖ version_u8 ‖ kind_u8 ‖ source_kind_u8 ‖ feed_id[32] ‖ price_account[32] ‖ offset_bps_i32_le ‖ max_conf_bps_u16_le ‖ band_bps_u16_le ‖ window_secs_u16_le ‖ max_age_secs_u16_le ‖ commit_open_i64_le ‖ commit_close_i64_le ‖ reference_time_i64_le ‖ outcome_time_i64_le)` — ohne Schwelle und ohne Text. Alles, was die Auswertung bestimmt, steht darin.
...
- **Regel O1 (DECISIONS 18./19.09.2026): die erste gültige Einreichung gewinnt.** Mit R = `reference_time` (Saison 1: 04:02 UTC, zwei Minuten nach Siegelschluss) ist eine Einreichung gültig, wenn `R ≤ now ≤ R + W`, das Update `Full` ist, die Feed-ID stimmt, `now − publish_time ≤ A`, `price > 0` und `conf·10 000 / price ≤ max_conf_bps`. Fixiert wird, was in diesem Moment im benannten Konto steht. Ein Wert von kurz vor R ist zulässig, solange er nicht älter als A ist — wegen `R − A > commit_close` aber nie einer, der vor Siegelschluss veröffentlicht wurde.
...
- Kein Catch-up-Sonderfall: außerhalb `[outcome_time, reveal_close)` gibt es keinen Reveal. Mit 72 h sind beim Commit von R die Runden R−1, R−2 und R−3 offen — mehr nicht, weil `reveal_close(R−4)` genau mit dem Öffnen des Siegelfensters von R zusammenfällt. Missing wird nicht gebucht, sondern abgeleitet.
...
- Speichert zusätzlich `outcome_margin_bps` = (MOVE: `|Bewegung|`, ABOVE: `Bewegung`) − `offset_bps`, mit Bewegung = `(out − ref)·10 000 / ref`, Rundung zur Null, auf i32 begrenzt. Positiv heißt „Ja um so viele Basispunkte", negativ „Nein um so viele". Der Ausgang selbst wird exakt entschieden; ein Margin von 0 heißt also „innerhalb eines Basispunkts an der Schwelle", nicht zwingend Nein.
- **Knappe Runden sind damit nachrechenbar:** `|outcome_margin_bps| ≤ band_bps` (beides on-chain, `band_bps` steht vorab in den Bedingungen und damit im `terms_hash`). Saison 1: 25 bps, aufgerundet aus dem p90 der gemessenen Spanne je Zeitpunkt (SOL 22,7; ETH 15,3; BTC 11,6 — docs/spikes/baserate.md). Die App zeigt es an, jeder kann es selbst prüfen.
...
Tägliche Transaktion des Spielers = `[reveal(r) für jede offene Runde r, score_entry(r) falls Resolved, commit(R)]` — eine Freigabe deckt alles Offene auf und siegelt heute. Gemessen (tests/capacity.rs): drei Aufdeckungen + Siegeln + zwei Memos = 1 027 von 1 232 Bytes, vier passen auch, bei fünf reißt die Transaktion. Rechenkosten dafür 74 596 CU. Jede Instruktion bleibt einzeln aufrufbar (fehlender Salt, annullierte Vorrunde, Pause). `set_reference` und `resolve` inklusive Oracle-Posting sind **nicht** Teil dieser Transaktion — sie kosten mehrere Freigaben und laufen standardmäßig über den Cron; in der App optional als eigener Knopf.

**Compute-Budget explizit setzen**, nicht auf den Standard verlassen: gemessen `commit` 28 868 CU + `reveal` 16 032 CU ≈ 45 000 CU für die tägliche Transaktion mit einer Aufdeckung, 74 596 CU mit dreien (LiteSVM, ohne `score_entry`; mit `score_entry` ≈ 55 000). Client setzt `ComputeBudgetInstruction::set_compute_unit_limit` auf den gemessenen Wert + 20 % und dazu den Priority-Fee-Preis (02 §Transaktions-Landing).
...
| Spätes Versiegeln mit Wissen über die Referenz | Referenz um `reference_time` = Siegelschluss + 2 min (04:02). `create_round` verlangt `reference_time − A > commit_close`, also ist jeder zulässige Referenzwert nach 04:01 veröffentlicht — eine volle Minute nach Siegelschluss. Niemand, der versiegelt hat, kann ihn gesehen haben (Test `no_admissible_reference_was_visible_while_sealing`) |
